...
async function main() {
  ...
  if (process.env.DEVNET_PRIVATE_KEY) {
    ...
    console.log(
      chalk.yellow("⚠️  Using ephemeral wallet (no private key in .env)"),
    );
    console.log(
      chalk.gray("   Add DEVNET_PRIVATE_KEY to .env to use your wallet"),
    );
  }
      ...
      for (const instruction of idl.instructions || []) {
        ...
        if (instruction.args && instruction.args.length > 0) {
          ...
          for (const arg of instruction.args) {
            console.log(
              chalk.gray(`       - ${arg.name}: ${JSON.stringify(arg.type)}`),
            );
          }
        }
      }
...
const liquidityResult = await client.pools.addLiquidity({
  ...
  console.log(
    chalk.gray("3. Use the SDK methods to swap, add/remove liquidity"),
  );
