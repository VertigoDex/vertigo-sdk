...
async function main() {
  ...
  try {
    ...
    const mockPrograms = {
      ...
      SPL_TOKEN_FACTORY: generateMockProgramId(
        "vertigo-spl-token-factory-devnet",
      ),
      TOKEN_2022_FACTORY: generateMockProgramId(
        "vertigo-token-2022-factory-devnet",
      ),
      PERMISSIONED_RELAY: generateMockProgramId(
        "vertigo-permissioned-relay-devnet",
      ),
    };
    ...
    try {
      ...
      console.log(
        chalk.yellow(
          "Note: Full client initialization requires deployed programs",
        ),
      );
      console.log(
        chalk.yellow("Mock test verifies SDK structure and imports only"),
      );
      ...
      console.log(
        chalk.gray(
          "This is normal - programs need to be deployed for full functionality",
        ),
      );
    }
    ...
    console.log(
      chalk.gray("1. Get the actual program IDs from the Vertigo team"),
    );
    console.log(
      chalk.gray("2. Update src/core/constants.ts with real addresses"),
    );
    console.log(chalk.gray("3. Run: yarn test:devnet"));

    console.log(chalk.blue("\n💡 Alternative: Use existing AMMs for testing"));
    console.log(
      chalk.gray("You could test with Orca or Raydium devnet programs:"),
    );
    console.log(
      chalk.gray(
    ...
    console.log(
      chalk.gray("- Raydium AMM: HWy1jotHpo6UqeQxx49dpYYdQB8wj9Qk9MdxwjLvDHB8"),
    );
  } catch (error: any) {
