...
class IntegrationTestSuite {
  ...
  async testProgramConnectivity() {
    ...
    for (const [name, programId] of Object.entries(PROGRAMS)) {
      ...
      try {
        const accountInfo =
          await this.context.connection.getAccountInfo(programId);
  ...
  async testAccountDiscovery() {
    ...
    try {
      ...
      if (pools.length > 0) {
        ...
        console.log(
          chalk.green(`✅ Found ${pools.length} pools on AMM program`),
        );

        // Display first pool
        console.log(
          chalk.gray(`   Sample pool: ${pools[0].pubkey.toBase58()}`),
        );
      } else {
        this.addResult(
          "Pool Discovery",
      ...
      if (tokenAccounts.length > 0) {
        console.log(
          chalk.green(`✅ Found ${tokenAccounts.length} token accounts`),
        );
        for (const account of tokenAccounts.slice(0, 3)) {
          console.log(
            chalk.gray(
              `   Token: ${account.mint.toBase58()} | Amount: ${account.amount}`,
            ),
          );
        }
      } else {
  ...
  async testPoolOperations() {
    ...
    try {
      ...
      if (poolAccount) {
        ...
        console.log(
          chalk.yellow("⚠️  Pool does not exist (would need to be created)"),
        );
        this.addResult(
          "Pool PDA Derivation",
          "passed",
  ...
  async testPoolAuthority() {
    ...
    try {
      ...
      const poolAuthInfo = await this.context.connection.getAccountInfo(
      ...
      if (poolAuthInfo && poolAuthInfo.executable) {
        ...
        const authorityAccount =
          await this.context.connection.getAccountInfo(authorityPda);
        if (authorityAccount) {
          console.log(chalk.green("✅ Authority account exists"));
        } else {
          console.log(
            chalk.yellow("⚠️  No authority account found for this wallet"),
          );
          console.log(
            chalk.gray("   This is normal if you haven't created one yet"),
          );
        }
      } else {
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          console.log(
            chalk.green(`✅ Token A liquidity added: ${transferSig}`),
          );
          ...
          console.log(
            chalk.gray(`   Vault A: ${vaultABalance.value.uiAmount} Token A`),
          );
          console.log(
            chalk.gray(`   Vault B: ${vaultBBalance.value.uiAmount} Token B`),
          );
          ...
          console.log(
            chalk.yellow(
              `⚠️  Pool creation transaction failed: ${txError.message}`,
            ),
          );
          console.log(
            chalk.gray(
              "   This might be due to IDL/instruction encoding issues",
            ),
          );
          console.log(
            chalk.gray(
              "   Pool can still be created manually with correct parameters",
            ),
          );
        ...
        console.log(
          chalk.yellow(
            `⚠️  Pool creation preparation failed: ${error.message}`,
          ),
        );
        this.addResult(
          "Pool Initialization",
  ...
  async testSwapSimulation() {
    ...
    try {
      ...
      if (this.context.createdPool) {
        ...
        console.log(
          chalk.gray(`   Pool: ${this.context.createdPool.address.toBase58()}`),
        );
        console.log(
          chalk.gray(
            `   Token A: ${this.context.createdPool.mintA.toBase58()}`,
          ),
        );
        console.log(
          chalk.gray(
            `   Token B: ${this.context.createdPool.mintB.toBase58()}`,
          ),
        );

        // Check if pool actually exists on-chain
        const poolAccount = await this.context.connection.getAccountInfo(
          ...
          if (this.context.ammClient.hasIdl()) {
            try {
              ...
              const initialBalances = {
                tokenA: await this.context.connection.getTokenAccountBalance(
                  this.context.createdPool.tokenAccountA,
                ),
                tokenB: await this.context.connection.getTokenAccountBalance(
                  this.context.createdPool.tokenAccountB,
                ),
              };
              ...
              console.log(
                chalk.gray(
                  `   Token A: ${initialBalances.tokenA.value.uiAmount}`,
                ),
              );
              console.log(
                chalk.gray(
                  `   Token B: ${initialBalances.tokenB.value.uiAmount}`,
                ),
              );
              ...
              const vaultABalance =
                await this.context.connection.getTokenAccountBalance(vaultA);
              const vaultBBalance =
                await this.context.connection.getTokenAccountBalance(vaultB);
              console.log(
                chalk.gray(
                  `   Vault A balance: ${vaultABalance.value.uiAmount} Token A`,
                ),
              );
              console.log(
                chalk.gray(
                  `   Vault B balance: ${vaultBBalance.value.uiAmount} Token B`,
                ),
              );

              // Check pool state to understand its reserves
              const poolData = await this.context.connection.getAccountInfo(
                this.context.createdPool.address,
              );
              if (poolData && poolData.data) {
                ...
                try {
                  ...
                  if (dataView.byteLength >= reservesOffset + 16) {
                    const tokenAReserves = dataView.getBigUint64(
                      reservesOffset,
                      true,
                    );
                    const tokenBReserves = dataView.getBigUint64(
                      reservesOffset + 8,
                      true,
                    );
                    console.log(
                      chalk.gray(
                        `   Token A reserves in pool: ${tokenAReserves}`,
                      ),
                    );
                    console.log(
                      chalk.gray(
                        `   Token B reserves in pool: ${tokenBReserves}`,
                      ),
                    );
                  }
                } catch (e) {
              ...
              const buyInstruction = this.context.ammClient.buildInstruction(
                ...
                [
                  {
                    pubkey: this.context.createdPool.address,
                    isSigner: false,
                    isWritable: true,
                  },
                  {
                    pubkey: this.context.wallet.publicKey,
                    isSigner: true,
                    isWritable: false,
                  },
                  {
                  ...
                  {
                    pubkey: this.context.createdPool.mintA,
                    isSigner: false,
                    isWritable: false,
                  },
                  {
                    pubkey: this.context.createdPool.mintB,
                    isSigner: false,
                    isWritable: false,
                  },
                  {
                    pubkey: this.context.createdPool.tokenAccountA,
                    isSigner: false,
                    isWritable: true,
                  },
                  {
                    pubkey: this.context.createdPool.tokenAccountB,
                    isSigner: false,
                    isWritable: true,
                  },
              ...
              try {
                ...
                console.log(
                  chalk.green(`✅ BUY swap executed successfully: ${buySig}`),
                );
                console.log(
                  chalk.gray(
                    `   View on explorer: https://explorer.solana.com/tx/${buySig}?cluster=devnet`,
                  ),
                );
                ...
                console.log(
                  chalk.gray(
                    `   Token A: ${finalBalances.tokenA.value.uiAmount} (was ${initialBalances.tokenA.value.uiAmount})`,
                  ),
                );
                console.log(
                  chalk.gray(
                    `   Token B: ${finalBalances.tokenB.value.uiAmount} (was ${initialBalances.tokenB.value.uiAmount})`,
                  ),
                );

                const tokenAChange =
                  (finalBalances.tokenA.value.uiAmount || 0) -
                  (initialBalances.tokenA.value.uiAmount || 0);
                const tokenBChange =
                  (finalBalances.tokenB.value.uiAmount || 0) -
                  (initialBalances.tokenB.value.uiAmount || 0);

                console.log(chalk.green("\n✅ Swap executed successfully!"));
                console.log(
                  chalk.gray(
                    `   Received: ${tokenAChange > 0 ? "+" : ""}${tokenAChange} Token A`,
                  ),
                );
                ...
                console.log(
                  chalk.yellow("🚀 Executing SELL swap on devnet..."),
                );
                ...
                console.log(
                  chalk.green(`✅ SELL swap executed successfully: ${sellSig}`),
                );
                console.log(
                  chalk.gray(
                    `   View on explorer: https://explorer.solana.com/tx/${sellSig}?cluster=devnet`,
                  ),
                );
                ...
                console.log(
                  chalk.yellow(`⚠️  Swap failed: ${swapError.message}`),
                );
                console.log(
                  chalk.gray(
                    "   The AMM bonding curve may require specific liquidity ratios",
                  ),
                );
              ...
              console.log(
                chalk.red(`❌ Swap execution failed: ${error.message}`),
              );
              this.addResult(
                "Swap Execution",
                "failed",
      ...
      if (testPools.length > 0) {
        ...
        for (const pool of testPools) {
          ...
          console.log(
            chalk.gray(`   Amount In: ${swapParams.amountIn.toString()}`),
          );
          console.log(
            chalk.gray(`   Slippage: ${swapParams.slippageBps / 100}%`),
          );
          ...
          if (poolAccount) {
            console.log(chalk.green("✅ Pool verified on-chain"));
            console.log(
              chalk.gray(`   Data length: ${poolAccount.data.length} bytes`),
            );
            console.log(
              chalk.gray(`   Owner: ${poolAccount.owner.toBase58()}`),
            );
      ...
      console.log(
        chalk.gray(`   Token In: ${swapParams.tokenIn.toBase58()} (SOL)`),
      );
      console.log(
        chalk.gray(`   Token Out: ${swapParams.tokenOut.toBase58()} (USDC)`),
      );
      console.log(
        chalk.gray(`   Amount In: ${swapParams.amountIn.toString()} lamports`),
      );
      ...
      if (!poolExists) {
        ...
        console.log(
          chalk.gray(
            "   Tip: Create a pool using the test tokens from Pool Initialization",
          ),
        );
        ...
        const tokenAccounts =
          await this.context.accountFinder.findTokenAccounts(
            this.context.wallet.publicKey,
          );
        ...
        if (tokenAccounts.length > 0) {
          for (const account of tokenAccounts.slice(0, 3)) {
            if (Number(account.amount) > 0) {
              console.log(
                chalk.gray(
                  `     • ${account.mint.toBase58()} (Balance: ${account.amount})`,
                ),
              );
            }
          }
   ...
   */
  private async findTestPools(): Promise<
    Array<{
      address: PublicKey;
      tokenA?: PublicKey;
      tokenB?: PublicKey;
    ...
    try {
      ...
      for (let i = 0; i < tokenAccounts.length; i++) {
        for (let j = i + 1; j < tokenAccounts.length; j++) {
          ...
          if (
            Number(tokenAccounts[i].amount) === 0 ||
            Number(tokenAccounts[j].amount) === 0
          ) {
            continue;
          }
          ...
          // Check if pool exists
          const poolAccount =
            await this.context.connection.getAccountInfo(poolPda);
          if (poolAccount) {
            testPools.push({
              address: poolPda,
              tokenA,
...
async function main() {
  try {
    ...
    if (
      ...
      process.env.USE_EXISTING_WALLET === "true"
    ) {
      ...
      console.log(
        chalk.gray(
          `Private key (for debugging): ${bs58.encode(wallet.secretKey)}`,
        ),
      );
      console.log(
        chalk.gray(
          "Note: This is a temporary test wallet, do not send real funds to it!",
        ),
      );
    }
    ...
    if (needsAirdrop) {
      ...
      for (let i = 0; i < maxRetries && !airdropSuccess; i++) {
        try {
          ...
          if (balance > 0) {
            ...
            console.log(
              chalk.gray(`Balance: ${balance / LAMPORTS_PER_SOL} SOL`),
            );
            airdropSuccess = true;
          }
          ...
          if (i === maxRetries - 1) {
            console.log(
              chalk.yellow(`⚠️  Airdrop failed after ${maxRetries} attempts`),
            );
          }
        }
      }
      ...
      if (!airdropSuccess) {
        console.log(
          chalk.yellow("⚠️  Airdrop failed, trying alternative approach..."),
        );
        ...
        if (process.env.DEVNET_PRIVATE_KEY) {
          ...
          if (envBalance > LAMPORTS_PER_SOL) {
            ...
            console.log(
              chalk.gray("Transferring SOL from environment wallet..."),
            );
            ...
            await sendAndConfirmTransaction(connection, transferTx, [
              envWallet,
            ]);
        ...
        if (finalBalance === 0) {
          throw new Error(
            "Unable to fund test wallet. Please try again later or use a funded wallet.",
          );
        }
        console.log(
          chalk.gray(`Final balance: ${finalBalance / LAMPORTS_PER_SOL} SOL`),
        );
      }
