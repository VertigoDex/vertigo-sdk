...
class VertigoTestRunner {
      ...
      tests: [
        ...
        },
        {
          ...
          fn: async () => {
            ...
            const price = await this.client.pools.getPoolPrice(
              pools[0].publicKey,
            );
            return {
              ...
              isReciprocal:
                Math.abs(price.priceAtoB * price.priceBtoA - 1) < 0.001,
            };
          },
        },
      ],
        ...
        },
        {
          ...
          fn: async () => {
            ...
            return {
              ...
              invalidIsInvalid:
                !this.client.factory.validateMetadata(invalidMetadata),
            };
          },
        },
        {
      ...
      tests: [
        ...
        },
        {
          ...
          fn: async () => {
            ...
            try {
              const stats = await this.client.api.getPoolStats(
                pools[0].publicKey.toBase58(),
              );
              return {
              ...
              return {
                hasStats: false,
                note: "Stats may not be available on devnet",
              };
            }
          },
        },
  ...
  private log(
    message: string,
    color: keyof typeof chalk = "white",
    bold: boolean = false,
  ) {
  ...
  .action(async (options) => {
    try {
      ...
      if (options.category) {
        const suite = runner
          .getTestSuites()
          .find((s) => s.name.toLowerCase() === options.category.toLowerCase());
