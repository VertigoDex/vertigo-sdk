...
describe('PoolClient Integration Tests (Devnet)', () => {
  ...
  describe('Pool Discovery', () => {
    ...
    it('should fetch pool by address', async () => {
      ...
      expect(fetchedPool.account.mintA.equals(targetPool.account.mintA)).toBe(true);
      expect(fetchedPool.account.mintB.equals(targetPool.account.mintB)).toBe(true);
    });
    ...
    it('should find pools by token mints', async () => {
      ...
      const hasMintA = poolsWithMintA.some(pool => 
        pool.account.mintA.equals(mintA) || pool.account.mintB.equals(mintA)
      );
      expect(hasMintA).toBe(true);
      
      // Find pools with both mints
      const poolsWithBothMints = await poolClient.findPoolsByMints(mintA, mintB);
      expect(poolsWithBothMints.length).toBeGreaterThan(0);
      
      const hasCorrectPair = poolsWithBothMints.some(pool =>
        (pool.account.mintA.equals(mintA) && pool.account.mintB.equals(mintB)) ||
        (pool.account.mintA.equals(mintB) && pool.account.mintB.equals(mintA))
      );
    ...
    it('should handle non-existent pool gracefully', async () => {
      ...
      await expect(
        poolClient.getPool(nonExistentPool)
      ).rejects.toThrow();
    });
  });
  ...
  describe('Pool Information', () => {
    ...
    it('should get pool price', async () => {
      ...
      expect(Math.abs(price.priceAtoB * price.priceBtoA - 1)).toBeLessThan(tolerance);
    });
  ...
  describe('Pool Analytics', () => {
    ...
    it('should get pool fees collected', async () => {
      ...
      try {
        const fees = await poolClient.getPoolFeesCollected(pool.publicKey, '24h');
    ...
    it('should sort pools by volume', async () => {
      ...
      try {
        ...
        for (let i = 1; i < sortedPools.length; i++) {
          const prevVolume = await poolClient.getPoolVolume(sortedPools[i - 1].publicKey, '24h');
          const currVolume = await poolClient.getPoolVolume(sortedPools[i].publicKey, '24h');
          
          expect(prevVolume.volumeUSD).toBeGreaterThanOrEqual(currVolume.volumeUSD);
        }
      } catch (error) {
  ...
  describe('Error Handling', () => {
    it('should handle invalid pool address', async () => {
      ...
      await expect(
        poolClient.getPool(invalidAddress)
      ).rejects.toThrow();
    });
