...
async function createTestPool() {
  ...
  try {
    ...
    if (balance < 0.1 * LAMPORTS_PER_SOL) {
      ...
      console.log(
        chalk.gray(
          `solana airdrop 5 ${wallet.publicKey.toBase58()} --url devnet`,
        ),
      );
      return;
    }
    ...
    console.log(
      chalk.gray(
        `Token Account A: ${tokenAccountA.address.toBase58().slice(0, 16)}...`,
      ),
    );
    ...
    console.log(
      chalk.gray(
        `Token Account B: ${tokenAccountB.address.toBase58().slice(0, 16)}...`,
      ),
    );
    ...
    if (poolAccount) {
      ...
      console.log(
        chalk.gray(
          `  Lamports: ${poolAccount.lamports / LAMPORTS_PER_SOL} SOL`,
        ),
      );
      return;
    }
    ...
    console.log(
      chalk.gray(
        `Vault A: ${vaultA.toBase58().slice(0, 16)}... (bump: ${vaultABump})`,
      ),
    );
    console.log(
      chalk.gray(
        `Vault B: ${vaultB.toBase58().slice(0, 16)}... (bump: ${vaultBBump})`,
      ),
    );
    ...
    console.log(
      chalk.gray("   1. Proper instruction encoding with Anchor IDL"),
    );
    ...
    console.log(
      chalk.gray("You can now use these tokens to create and test pools."),
    );
  } catch (error: any) {
