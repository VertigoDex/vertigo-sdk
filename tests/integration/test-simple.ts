...
async function main() {
  ...
  try {
    ...
    const programs = [
      ...
      { name: 'Pool Authority', address: 'Vert8xR82wYZrftk7PLcYJNxSdhLCCAMy7zbCNNSS4d' },
      { name: 'Permissioned Relay', address: 'FREEXMQuGxqrmT8gKJt82zfJjZACBiR8MNuQCbGSHyzF' },
      { name: 'SPL Token Factory', address: 'FactRtzKDU69a88rVZbnTofJFSVSDtzEHQG36NigvJjS' },
      { name: 'Token 2022 Factory', address: 'FAcTgvrF2jAiPnWEZGLCV3PTQJrypa8GXLFowBv8T4Vs' },
    ];
    ...
    for (const program of programs) {
      try {
        ...
        if (accountInfo && accountInfo.executable) {
          console.log(chalk.green(`✅ ${program.name}: Deployed and executable`));
        } else {
          console.log(chalk.red(`❌ ${program.name}: Not found or not executable`));
        }
      } catch (error: any) {
        console.log(chalk.red(`❌ ${program.name}: Error checking - ${error.message}`));
      }
    }
    ...
    try {
      const ammProgramId = new PublicKey('vrTGoBuy5rYSxAfV3jaRJWHH6nN9WK4NRExGxsk1bCJ');
      const idl = await anchor.Program.fetchIdl(ammProgramId, provider);
      
      if (idl) {
        console.log(chalk.green('✅ AMM IDL fetched from chain'));
        console.log(chalk.gray(`   Instructions: ${idl.instructions?.length || 0}`));
    ...
    const apiTests = [
      {
        ...
        fn: async () => {
          try {
            ...
            return { success: false, note: 'API may not be available on devnet' };
          }
        }
      },
      {
        ...
        fn: async () => {
          try {
            const price = await api.getTokenPrice('So11111111111111111111111111111111111111112');
            return { success: true, price: price.price };
          } catch (error) {
            return { success: false, note: 'Price data may not be available on devnet' };
          }
        }
      }
    ];
    ...
    for (const test of apiTests) {
      ...
      if (result.success) {
        ...
        if (result.data || result.price) {
          console.log(chalk.gray(`   Result: ${JSON.stringify(result.data || result.price)}`));
        }
      } else {
    ...
    const { formatTokenAmount, parseTokenAmount } = await import('../../src/utils');
    const BN = anchor.BN;
    ...
    console.log(chalk.yellow('⚠️  Full integration tests require deployed programs'));
