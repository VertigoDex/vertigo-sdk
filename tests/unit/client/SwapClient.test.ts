...
import { DEFAULT_SLIPPAGE_BPS, MAX_SLIPPAGE_BPS } from '../../../src/core/constants';
...
describe('SwapClient', () => {
  ...
  describe('getQuote', () => {
    ...
    it('should try reverse pool when direct pool not found', async () => {
      mockPoolClient.findPoolsByMints = vi.fn()
  ...
  describe('simulateSwap', () => {
    ...
    it('should handle simulation exception', async () => {
      mockClient.connection.simulateTransaction = vi.fn().mockRejectedValue(
  ...
  describe('buildSwapTransaction', () => {
    ...
    it('should estimate priority fee when auto', async () => {
      mockClient.connection.getRecentPrioritizationFees = vi.fn().mockResolvedValue([
